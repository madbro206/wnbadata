...
players_data <- wnba_player_box %>%
  filter(athlete_display_name %in% c("Ezi Magbegor", "Cameron Brink")) %>%
  arrange(game_date)
...
custom_colors <- c("Ezi Magbegor" = "darkgreen", "Cameron Brink" = "purple")
...
ggplot(players_data, aes(x = game_date, y = cumulative_blocks, color = athlete_display_name, group = athlete_display_name)) +
  ...
  labs(title = "2024 Cumulative Blocks by Date for Ezi Magbegor and Cameron Brink", 
       subtitle = "through 6/2/2024, via wehoop",
       x = "Game Date", y = "Cumulative Blocks") +
  theme_minimal() +
  theme(legend.title = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotating x-axis labels for better readability
  geom_vline(xintercept = as.numeric(game_dates), color = "darkgray", linetype = "solid", alpha = 0.3)  # Adding vertical lines
