...
#uw <- subset(wbb_pbp, (home_team_name=="Washington" | away_team_name=="Washington") & (qtr==1 | qtr==2 | qtr==3 | qtr==4))

#uw games that they lost
uw <- subset(wbb_team_box, team_location=="Washington" & opponent_team_score > team_score)
uw$ft_missed <- uw$free_throws_attempted-uw$free_throws_made
uw$margin <- uw$opponent_team_score - uw$team_score

uw_ft <- uw %>%
  select(game_date, opponent_team_location, opponent_team_name, ft_missed, margin) %>%
  mutate(ft_loss = ft_missed >= margin)

mean(uw$ft_missed)

#just to display overtime games
subset(uw_ft, game_date=="2024-2-18"|game_date=="2024-2-16"|game_date=="2024-2-09")