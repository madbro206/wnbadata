...
pac_key <- cbind(team_location= pac_teams, key=c(1:12))
...
pac <- merge(pac, pac_key, by.x = "team_location", by.y = "team_location", all.x = TRUE)
pac <- merge(pac, pac_key, by.x = "opponent_team_location", by.y = "team_location", all.x = TRUE)
...
ranked_teams <- cbind(pac_teams, pagerank)
...
#make pie
pie(ranked_teams$pagerank, labels = ranked_teams$pac_teams, main = "PageRank Distribution of Pac-12 Teams 2024")
